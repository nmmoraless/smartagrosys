...
export class UsuariosComponent {
  ...
  private _UsuarioService: any;
    ...
    ngOnInit(): void {
      ...
      const usuario = this._usuario.getUsuarios();
      const rol = this._rol.getRoles();
      const estado = this.__estado.getEstados();
      forkJoin([usuario, rol, estado]).subscribe(([usuarios, roles, estados]) => {
        console.log(usuarios, roles, estados);
